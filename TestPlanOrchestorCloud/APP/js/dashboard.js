...
function renderModels() {
  ...
  const modelData = [
    { key: 'openrouter', icon: '🌌', name: 'OpenRouter', sub: 'Hub API', bg: 'rgba(59,130,246,0.12)' },
    { key: 'deepseek', icon: '🐳', name: 'DeepSeek', sub: 'Deep V2 API', bg: 'rgba(16,185,129,0.12)' },
    { key: 'mistral', icon: '🌪️', name: 'Mistral', sub: 'Mistral AI', bg: 'rgba(245,158,11,0.12)' },
    { key: 'groq', icon: '⚡', name: 'Groq', sub: 'Llama 3 Inference', bg: 'rgba(245,158,11,0.12)' }
  ];
...
function populateModelDropdown(model) {
  ...
  let opts = '';
  if(model === 'openrouter') {
    opts = '<option value="openai/gpt-4o">GPT-4o (OpenRouter)</option><option value="anthropic/claude-3.5-sonnet">Claude 3.5 Sonnet</option><option value="meta-llama/llama-3.1-70b-instruct">Llama 3.1 70B</option>';
  } else if (model === 'deepseek') {
    opts = '<option value="deepseek-chat">DeepSeek Chat</option><option value="deepseek-reasoner">DeepSeek Reasoner</option>';
  } else if (model === 'mistral') {
    opts = '<option value="mistral-large-latest">Mistral Large</option><option value="ministral-8b-latest">Ministral 8B</option>';
  } else {
    opts = '<option value="llama-3.1-8b-instant">Llama 3.1 8B</option><option value="llama3-70b-8192">Llama 3 70B</option>';
  }
