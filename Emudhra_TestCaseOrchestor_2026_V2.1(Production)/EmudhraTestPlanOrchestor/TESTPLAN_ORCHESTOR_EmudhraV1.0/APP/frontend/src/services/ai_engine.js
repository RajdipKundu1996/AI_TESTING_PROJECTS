...
- Output ONLY the JSON — nothing else.`;
    ...
    async generateJSON(problem, modelConfig) {
        ...
        if (!config || (!['ollama', 'mistral', 'groq'].includes(engine) && !config.apiKey)) {
    ...
    // Build a normalised call config for any engine (shared by both generate methods)
    _buildCallConfig(engine, modelConfig) {
        const config = modelConfig.data[engine] || {};
        if (!config || (!['ollama', 'mistral', 'groq'].includes(engine) && !config.apiKey)) {
            throw new Error(`API key missing for ${engine.toUpperCase()}. Please configure it in Dashboard settings.`);
        }
        ...
        return { ...config, baseUrl, version: config.version };
    },

    // Send a fully pre-composed prompt directly to the engine without any additional wrapping.
    // Use this whenever the caller has already built its own prompt (legacy single-string path).
    async generateWithPrompt(customPrompt, modelConfig, onChunk) {
        const engine = modelConfig.current;
        const callConfig = this._buildCallConfig(engine, modelConfig);
        // Wrap the single string as a plain user message (backward-compatible)
        ...
        switch (engine) {
            ...
            case 'openai':
            case 'groq':
                return await this.callOpenAICompatible(messages, callConfig, onChunk, engine.toUpperCase());
            case 'sarvam':
                return await this.callSarvam(messages, callConfig, onChunk);
            case 'gemini':
    ...
    // Send with proper system / user / assistant role separation.
    // systemPrompt  — string, sent as role:'system'
    // chatHistory   — array of {role:'user'|'assistant', content:string} (prior turns)
    // userText      — string, the current user message (sent as role:'user')
    async generateWithMessages(systemPrompt, chatHistory, userText, modelConfig, onChunk) {
        const engine = modelConfig.current;
        const callConfig = this._buildCallConfig(engine, modelConfig);
        ...
        switch (engine) {
            ...
            case 'sarvam':
                return await this.callSarvam(messages, callConfig, onChunk);
            case 'gemini':
                // Gemini uses its own format; flatten to a single prompt for now
                return await this.callGemini((systemPrompt ? systemPrompt + '\n\n' : '') + userText, callConfig, onChunk);
            case 'huggingface':
                return await this.callHuggingFace((systemPrompt ? systemPrompt + '\n\n' : '') + userText, callConfig, onChunk);
            case 'anthropic':
                return await this.callAnthropic((systemPrompt ? systemPrompt + '\n\n' : '') + userText, callConfig, onChunk);
            default:
                throw new Error('Unknown AI Engine selected.');
        }
    },
    ...
    async callSarvam(messagesOrPrompt, config, onChunk) {
        ...
        let messages = Array.isArray(messagesOrPrompt)
            ? [...messagesOrPrompt]
            : [{ role: 'user', content: messagesOrPrompt }];

        // Sarvam AI does not reliably honour role:'system'. Inject the system
        // prompt as a prefix on the FIRST user message so language/persona
        // instructions are never silently dropped.
        const sysIdx = messages.findIndex(m => m.role === 'system');
        if (sysIdx !== -1) {
            const sysContent = messages[sysIdx].content;
            messages = messages.filter((_, i) => i !== sysIdx);
            ...
            if (firstUserIdx !== -1) {
                ...
                messages.unshift({ role: 'user', content: sysContent });
            }
        }
        try {
            const body = {
                model: config.version || 'sarvam-30b',
                messages,
                temperature: 0.1,
                stream: !!onChunk
            };
    ...
    async callOpenAICompatible(messagesOrPrompt, config, onChunk, engineName) {
        ...
        try {
            ...
            if (err.message.includes('Failed to fetch')) {
                const target = engineName === 'MISTRAL' ? 'the local relay on port 11435' : config.baseUrl;
                throw new Error(`Connection to ${engineName} failed. Please check if ${target} is reachable.`);
            }
            throw err;
        }
    },
    ...
    async checkGroq(config) {
        try {
            ...
            if (res.status === 401) return { status: 'inactive', message: 'Invalid Groq API Key' };
            if (res.status === 403) return { status: 'inactive', message: 'Groq API: Permission Denied' };
            return { status: 'inactive', message: `Groq Error ${res.status}` };
        } catch (e) {
...
`;
    ...
    async validateConnection(engine, config) {
        try {
            ...
            switch (engine) {
                case 'mistral':
                    return await this.checkMistral(sanitizedConfig);
                case 'ollama':
                ...
                case 'groq':
                    return await this.checkGroq(sanitizedConfig);
                default:
    ...
    async checkMistral(config) {
        try {
            ...
            if (res.status === 403) return { status: 'inactive', message: 'Mistral API: Permission Denied' };
            return { status: 'inactive', message: `Mistral Error ${res.status}` };
        } catch (e) {
            return { status: 'inactive', message: 'Relay server offline (port 11435)' };
        }
    },
