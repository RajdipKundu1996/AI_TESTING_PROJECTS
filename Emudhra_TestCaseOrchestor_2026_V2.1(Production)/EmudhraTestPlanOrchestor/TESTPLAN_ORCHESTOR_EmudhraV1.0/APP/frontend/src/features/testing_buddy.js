...
// Human-readable names (used in per-turn language reminder injected into user message)
const LANG_NAMES = {
  en: 'English', hi: 'Hindi',  bn: 'Bengali', mr: 'Marathi',
  te: 'Telugu',  ta: 'Tamil',  gu: 'Gujarati', ur: 'Urdu',
  kn: 'Kannada', od: 'Odia',   ml: 'Malayalam'
};

// Native-script confirmation phrases — used INSIDE the language mandate so the model
// recognises the target script unambiguously (avoids Hindi↔Bengali confusion in Sarvam).
const LANG_NATIVE = {
  hi: 'हिन्दी — कृपया अपना पूरा उत्तर हिन्दी में लिखें।',
  bn: 'বাংলা — অনুগ্রহ করে সম্পূর্ণ উত্তর বাংলায় লিখুন।',
  mr: 'मराठी — कृपया आपले संपूर्ण उत्तर मराठीत लिहा।',
  te: 'తెలుగు — దయచేసి మీ మొత్తం సమాధానం తెలుగులో రాయండి.',
  ta: 'தமிழ் — தயவுசெய்து உங்கள் முழு பதிலையும் தமிழில் எழுதுங்கள்.',
  gu: 'ગુજરાતી — કૃપા કરી તમારો સંપૂર્ણ જવાબ ગુજરાતીમાં લખો.',
  ur: 'اردو — براہ کرم اپنا پورا جواب اردو میں لکھیں۔',
  kn: 'ಕನ್ನಡ — ದಯವಿಟ್ಟು ನಿಮ್ಮ ಸಂಪೂರ್ಣ ಉತ್ತರವನ್ನು ಕನ್ನಡದಲ್ಲಿ ಬರೆಯಿರಿ.',
  od: 'ଓଡ଼ିଆ — ଦୟାକରି ଆପଣଙ୍କ ସମ୍ପୂର୍ଣ୍ଣ ଉତ୍ତର ଓଡ଼ିଆରେ ଲିଖନ୍ତୁ।',
  ml: 'മലയാളം — ദയവായി നിങ്ങളുടെ മുഴുവൻ ഉത്തരവും മലയാളത്തിൽ എഴുതുക.'
};

const LANG_INSTRUCTIONS = {
  en: '',
  hi: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Hindi (हिन्दी). हिन्दी में जवाब देना अनिवार्य है। All headings, explanations, bullet points, tables, and conclusions must be written in Hindi script (देवनागरी). NOT in Bengali, NOT in English. Technical terms (like "test case", "API", "Playwright") may keep their English names, but all surrounding text must be in Hindi.',
  bn: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Bengali (বাংলা). বাংলায় উত্তর দেওয়া বাধ্যতামূলক। All headings, explanations, bullet points, tables, and conclusions must be written in Bengali script (বাংলা হরফ). NOT in Hindi, NOT in English. Technical terms may keep their English names, but all surrounding text must be in Bengali.',
  mr: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Marathi (मराठी). मराठीत उत्तर देणे आवश्यक आहे। All text must be in Marathi Devanagari script. Technical terms may stay English.',
  te: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Telugu (తెలుగు). తెలుగులో సమాధానం ఇవ్వడం తప్పనిసరి. All text must be in Telugu script. Technical terms may stay English.',
  ta: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Tamil (தமிழ்). தமிழில் பதில் அளிப்பது கட்டாயம். All text must be in Tamil script. Technical terms may stay English.',
  gu: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Gujarati (ગુજરાતી). ગુજરાતીમાં જવાબ આપવો ફરજિયાત છે. All text must be in Gujarati script. Technical terms may stay English.',
  ur: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Urdu (اردو). اردو میں جواب دینا لازمی ہے۔ All text must be in Urdu/Nastaliq script. Technical terms may stay English.',
  kn: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Kannada (ಕನ್ನಡ). ಕನ್ನಡದಲ್ಲಿ ಉತ್ತರ ನೀಡುವುದು ಕಡ್ಡಾಯ. All text must be in Kannada script. Technical terms may stay English.',
  od: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Odia (ଓଡ଼ିଆ). ଓଡ଼ିଆରେ ଉତ୍ତର ଦେବା ବାଧ୍ୟତାମୂଳକ। All text must be in Odia script. Technical terms may stay English.',
  ml: 'LANGUAGE REQUIREMENT: You MUST respond entirely in Malayalam (മലയാളം). മലയാളത്തിൽ ഉത്തരം നൽകൽ നിർബന്ധമാണ്. All text must be in Malayalam script. Technical terms may stay English.',
};
...
Start with a relevant emoji. Add a "💡 **Pro tip:**" only if the response is technical (skip for simple answers).`;
    ...
    const nativeConfirm = LANG_NATIVE[langCode] || '';
    const langBlock = langCode !== 'en' ? `\
╔══════════════════════════════════════════════════════════════╗
║  ⚠ LANGUAGE MANDATE — ABSOLUTE HIGHEST PRIORITY RULE ⚠      ║
║                                                              ║
║  OUTPUT LANGUAGE: ${langName.padEnd(42, ' ')}║
║  ${nativeConfirm.padEnd(60, ' ')}║
║                                                              ║
║  RULES (non-negotiable):                                     ║
║  • EVERY word, sentence, heading, bullet must be ${langName.padEnd(12,' ')}║
║  • Do NOT write Bengali when Hindi is requested              ║
║  • Do NOT write Hindi when Bengali is requested              ║
║  • Do NOT default to English or Bengali unless selected      ║
║  • Technical terms (API, test case, Playwright) stay English ║
║  • All explanatory text: ${langName.padEnd(34, ' ')}║
╚══════════════════════════════════════════════════════════════╝

` : '';

    return `${langBlock}You are ${state.name}, an advanced AI QA companion (pronouns: ${pronoun}).
...
${knowledge}`;
  ...
  async function sendMessage(userText) {
    ...
    // Prior turns as proper role-separated message objects (already stored that way)
    const priorTurns = state.chatHistory.slice(-11, -1); // up to 10 prior turns

    // Layer 2: prepend a per-turn language reminder to the user's message.
    // This is critical — even if the user writes in English, the AI is explicitly
    // reminded of the language requirement on every single turn.
    // Native script is included so the model recognises the exact target script
    // without ambiguity (e.g. Hindi देवनागरी vs Bengali বাংলা).
    const langCode = state.lang || 'en';
    const langName = LANG_NAMES[langCode] || langCode;
    const nativeNote = LANG_NATIVE[langCode] ? ` ${LANG_NATIVE[langCode]}` : '';
    const augmentedUser = langCode !== 'en'
      ? `[⚠ MANDATORY LANGUAGE RULE — respond in ${langName} ONLY.${nativeNote} Your ENTIRE reply must use ${langName} script. Do NOT use Bengali when Hindi is selected. Do NOT use any language other than ${langName}.]\n\n${userText}`
      : userText;
    ...
    try {
      await AIEngine.generateWithMessages(sysPrompt, priorTurns, augmentedUser, config, (chunk) => {
        responseText += chunk;
        if (firstChunk) {
          firstChunk = false;
          removeTyping();
