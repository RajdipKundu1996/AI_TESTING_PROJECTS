...
(function () {
  ...
  var PAGES = {
    ...
    'enterprise.html': { name: 'Enterprise',      key: 'enterprise' },
  };
  function currPage() {
  ...
  var PAGE_TIPS = {
    home:       'Click any module card to jump straight to a feature, or use the top navigation bar.',
    autoflow:   'Click **Start Recording** to auto-capture steps in a real browser, then **Replay Test** to execute them.',
    analysis:   'Upload screenshots or test output for AI-powered defect detection and classification.',
    reports:    'Export any test run as Excel or PDF using the download buttons on each report card.',
    dashboard:  'Charts here update as you run tests — click any metric card for a detailed drill-down.',
    admin:      'Use **Invite User** to add teammates, assign roles (Admin, Tester, Viewer), and manage access.',
    enterprise: 'Bulk test execution and certificate management are available in the tabs on this page.',
    unknown:    'I can guide you to any feature — just describe what you are looking for!',
  };
  ...
  var PAGE_CHIPS = {
    ...
    enterprise: ['Bulk Test Help', 'Certificate Guide', 'Compliance Report', 'Open Admin'],
  ...
  function respond(text) {
    ...
    var tip = PAGE_TIPS[pg.key] || PAGE_TIPS.unknown;
    return {
  ...
  function openPanel() {
    ...
    if (box && box.children.length === 0) {
      ...
      var pg = currPage();
      var u  = userName();
      var tip = s.contextHints && PAGE_TIPS[pg.key]
        ? '\n\n💡 **Tip:** ' + PAGE_TIPS[pg.key]
        : '';
  ...
  function scheduleProactive() {
    ...
    setTimeout(function () {
      ...
      if (!g || !gt) return;
      var tip = PAGE_TIPS[currPage().key] || PAGE_TIPS.unknown;
      gt.innerHTML = '💡 <strong>Tip:</strong> ' + tip.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
      g.style.display = 'block';
