...
interface GenerateRequest {
  provider: string;
  config: string | { baseUrl?: string; apiKey?: string; defaultModel?: string };
  prompt?: string;
}

app.post('/api/generate', upload.single('file'), async (req: Request, res: Response) => {
  const { provider, config, prompt } = req.body as GenerateRequest;
  const file = req.file;
  const { baseUrl, apiKey, defaultModel } = typeof config === 'string' ? JSON.parse(config) : (typeof config === 'object' && config !== null ? config : {} as any);
...
app.post('/api/export-xlsx', async (req: Request, res: Response) => {
  const { testCases } = req.body as ExportRequest;
  if (!testCases) {
  ...
  try {
    console.log(`[Export] Received ${testCases.length} characters of test case data`);
    ...
    // More robust split logic — handles many common LLM separators
    const cases = testCases.split(/(?=\n(?:-?\s*)?(?:\*\*|Summary:|###|TC-\d+|Test Case:|\*\*Summary:))/i)
      .map(c => c.trim())
      .filter(c => c.length > 20);

    console.log(`[Export] Parsed ${cases.length} potential test cases`);

    cases.forEach((tc: string) => {
      const getValue = (key: string) => {
        // Enhanced regex for Markdown/Plaintext fields:
        // Handles:
        // 1. Optional bullet points: - Summary
        // 2. Markdown bolding/headers: **Summary** or ### Summary
        // 3. Optional colon: Summary:
        // 4. Multi-line content until the next field header
        const regex = new RegExp(`(?:-?\\s*)?(?:\\*\\*|###)?\\s*${key}\\s*(?:\\*\\*)?:?\\s*([\\s\\S]*?)(?=\\n(?:-?\\s*)?(?:\\*\\*|###)?[\\w\\s]+(?:\\*\\*)?:|$)`, 'i');
